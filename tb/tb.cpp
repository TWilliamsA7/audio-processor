...
int main(int argc, char** argv) {
    ...
    uint32_t fmt_size;
    find_chunk(input_wav, "fmt ", fmt_size);
    ...
    uint32_t data_size;
    find_chunk(input_wav, "data", data_size);
    ...
    dut->valid_in = 0;
    dut->bypass = 0;
    dut->gain = 0x40; // unity gain (1.0 in Q2.6)
    ...
    // Flip bypass on partway through to test the alignment fix
    while (!file_ended || pipeline_flush_cycles > 0) {
        ...
        if (!file_ended) {
            if (input_wav.read(sample_buffer, 3)) {
                ...
                dut->valid_in = 1;
                // toggle bypass at sample 5 to check for glitch/misalignment
                dut->bypass = (sample_idx >= 5) ? 1 : 0;
                sample_idx++;
            } else {
                file_ended = true;
                dut->audio_in = 0;
        ...
        if (dut->valid_out) {
            ...
            int32_t signed_out = (int32_t)(data_out << 8) >> 8; // sign-extend 24->32
            std::cout << "sample " << total_data_bytes/3 << " -> " << signed_out
                      << " (bypass=" << (int)dut->bypass << ")" << std::endl;
            char out_buffer[3];
